...
gulp.task('pot', function () {
    return gulp.src(['inc/*.php', 'sendpulse-newsletter.php'])
        .pipe(require('gulp-wp-pot')( {
            domain: 'sendpulse-newsletter',
            package: 'SendPulse Email Marketing Newsletter'
        } ))
        .pipe(gulp.dest('languages/sendpulse-newsletter.pot'));
});
...
gulp.task('zip', function () {
    gulp.src(['../sendpulse-newsletter/**/*', '!node_modules', '!node_modules/**', '!tests', '!tests/**', '!.travis.yml', '!phpcs.ruleset.xml', '!phpunit.xml.dist', '!bin', '!bin/**'], {base:"../"})
        .pipe(require('gulp-zip')('sendpulse-newsletter.zip'))
        .pipe(gulp.dest('../../dist'));
